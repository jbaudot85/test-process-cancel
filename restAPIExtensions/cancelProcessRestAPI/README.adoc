...
= cancelProcess REST API
REST API to manage process cancelation

The project **cancelProcessRestAPI** is a Bonita REST API extension for **Bonita {short-bonita-version}** version  **community** written in `groovy` language.
...
[source,bash]
----
git clone https://your.github.com/cancelProcessRestAPI.git
cd cancelProcessRestAPI/
mvn package
----

The build should produce a zip archive under the `target/` folder named `cancelProcessRestAPI-1.0.0-SNAPSHOT.zip`
...
Take the built artifact `cancelProcessRestAPI-1.0.0-SNAPSHOT.zip` and upload it to your Bonita platform (see {doc-url}/rest-api-extension-archetype#_deployment[rest-api-extension deployment])
